...
<div class="col-lg-12">
  <div class="card">
    <div class="card-body">
      <h4 class="card-title">Legal</h4><hr>
      <p class="card-description">
      	<a href="javascript:void(0)" data-toggle="modal" data-target=".bd-example-modal-lg" id="tambah_legal">Tambah data</a>
      </p>
      ...
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header">
            <h4 class="modal-title" id="exampleModalLongTitle">Offering Latter</h4>
...
<script type="text/javascript">
		...
		if (session_jabatan == 'legal' || session_jabatan == 'LEGAL'){
			$('#tambah_legal').show()
		}else{
			$('#tambah_legal').hide()
		}

		$('#showCaaApproval').on('click','#MemoAoKreditCa',function () {
				...
				/*setTimeout(function () {
					load()
					$('#tampalte').val('views/content/form/add-offering.php')
					$('#app').load('views/content/form/add-offering.php')
				},500)*/
				checkOrder(id_order)
		})
		...
		function checkOrder (id_order) {
			$.ajax({
				type:'GET',
				url:url+'Home/checkOrderInOffering/'+id_order,
				dataType:'JSON',
				beforeSend:function () {
					load()
				},
				success:function (res) {
					if (res.amount == 1) {
							swal({
				        title:'Info!',
				        text:'Data dengan id order '+id_order+' sudah memiliki Offering Letter.',
				        icon: "info"
					    });
					}else{
						setTimeout(function () {
							load()
							$('#tamplate').val('views/content/form/add-biaya.php')
							$('#app').load('views/content/form/add-biaya.php')
						},500)
					}
				}
			})
		...
		CAA_Aproval()
		function CAA_Aproval() {
				$.ajax({
						type:'GET',
            url:url+'Home/CaaApprovalOfferingLatter',
            dataType:'JSON',
            beforeSend: function(){
            	$('#showCaaApproval').html('<tr><td colspan="5" style="text-align: center;"><i class="mdi mdi-18px mdi-spin mdi-loading"></i> Mengambil data . . .</td></tr>')
              load()
            },
            success:function(data){
							console.log(data)
            	if (data.amount == 0) {
            			$('#showCaaApproval').html('<tr><td colspan="5" style="text-align: center;"><i class="mdi mdi-close"></i> Data Kosong !!</td></tr>')
            	}else {
            			var html = '';
									var i;
									for(i=0; i<data.length; i++){
											html += '<tr href="#" style="cursor: pointer;" id="MemoAoKreditCa" dataIdKC="'+data[i].id_kredit_checking+'" dataIdO="'+data[i].id_order+'" dataIdN="'+data[i].id_calon_debitur+'">'+
		...
		$('#showOffering').on('click','#detailof',function () {
			...
			paramsID1.val(id_order)
			load()
			$('#tamplate').val('views/content/data/detail-legal.php')
			$('#app').load('views/content/data/detail-legal.php')
		})

		$('#showOffering').on('click','#edit',function () {
			var id_order = $(this).attr('dataIdOr')
			paramsID1.val(id_order)
			load()
			$('#tamplate').val('views/content/form/edit-legal.php')
			$('#app').load('views/content/form/edit-legal.php')
		})
		...
		function OfferingLatter(startlimit,areakerja,keywsearch) {
            ...
            beforeSend: function(){
            	...
            	$('#selectAreaKerja').prop('disabled',true)
            	$('#search').prop('disabled',true)
            	$('#showOffering').html('<tr><td colspan="6" style="text-align: center;"><i class="mdi mdi-18px mdi-spin mdi-loading"></i> Mengambil data . . .</td></tr>')
              $('#pagination').hide()
              load()
            },
            success:function(data){
      ...
      $('#search').prop('disabled',false)
			if (data.amount == 0) {
					$('#pagination').hide()
    			$('#showOffering').html('<tr><td colspan="6" style="text-align: center;"><i class="mdi mdi-close"></i> Data Kosong !!</td></tr>')
    	}else {
					$('#pagination').show()
    			var html = '';
					var i;
					for(i=0; i<data.length; i++){
							...
							if (session_id_user == data[i].user_id){
		...
		if (session_jabatan == 'legal' || session_jabatan == 'LEGAL' || session_jabatan == 'ketua'){
