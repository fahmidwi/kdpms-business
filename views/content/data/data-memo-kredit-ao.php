...
<div class="col-lg-12">
  <div class="card">
    <div class="card-body">
      <h4 class="card-title">Data Memo kredit Ao</h4><hr>
      ...
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header">
            <h4 class="modal-title" id="exampleModalLongTitle">Survey siap memo Ao</h4>
                          ...
                          <!-- <div class="form-group" style="float:right;">
                            <div class="input-group">
                             <input type="text" class="form-control" placeholder="Cari Berdasarkan NIK Atau Nama" aria-describedby="colored-addon1" style="width: 80%; box-shadow:0.2px 0.2px 0.2px 0.2px grey;">
										          ...
										          <tbody id="showSurveyKomiteSudah">
										          </tbody>
														</table>
													</div>
                      </div>
...
<script type="text/javascript">
		...
		function checkOrder (id_order) {
			$.ajax({
				type:'GET',
				url:url+'Home/checkOrderInMemoAo/'+id_order,
				dataType:'JSON',
				beforeSend:() => {
					load()
				},
				success:function (res) {
					if (res.amount == 1) {
						swal({
						  title:'Info!',
						  text:'Data order dengan id '+id_order+' sudah memiliki data Memo Ao.',
						  icon: "info"
						});
					}else{
						setTimeout(function () {
		...
		SurveyKomite_readyAo()
		function SurveyKomite_readyAo() {
				$.ajax({
						type:'GET',
            url:url+'Home/SurveyKomiteAo',
            dataType:'JSON',
            beforeSend: function(){
            	$('#showSurveyKomiteSudah').html('<tr><td colspan="5" style="text-align: center;"><i class="mdi mdi-18px mdi-spin mdi-loading"></i> Mengambil data . . .</td></tr>')
              load()
            },
            success:function(data){
            	if (data.amount == 0) {
            			$('#showSurveyKomiteSudah').html('<tr><td colspan="5" style="text-align: center;"><i class="mdi mdi-close"></i> Data Kosong !!</td></tr>')
            	}else {
            			var html = '';
									var i;
									for(i=0; i<data.length; i++){
											html += '<tr href="#" style="cursor: pointer;" id="MemoAoKreditCheck" dataIdKC="'+data[i].id_kredit_checking+'" dataIdO="'+data[i].id_order+'" dataIdN="'+data[i].id_calon_debitur+'">'+
															'<td>'+data[i].id_kredit_checking+'</td>'+
									...
									$('#showSurveyKomiteSudah').html(html);
            	}
            },
		...
		$('#showMemoAo').on('click','#detailmemoao',function () {
			...
			load()
			$('#tamplate').val('views/content/data/detail-memo-ao.php')
			$('#app').load('views/content/data/detail-memo-ao.php')
		})

		$('#showMemoAo').on('click','#edit',function () {
			var id_order = $(this).attr('dataIdOr')

			paramsID2.val(id_order)
			load()
			$('#tamplate').val('views/content/form/edit-memo-kredit-ao.php')
			$('#app').load('views/content/form/edit-memo-kredit-ao.php')
		})
		...
		'use strict'
		function get_dataMemoAo(startlimit,areakerja,keysearch) {
				$.ajax({
						type:'GET',
            url:url+'Home/get_dataMemoAo/'+startlimit+'/'+areakerja+'/'+keysearch,
            dataType:'JSON',
            beforeSend: function(){
							totalData()
							$('#load_page').val('true');
            	$('#search').prop('disabled',true)
            	$('#showMemoAo').html('<tr><td colspan="8" style="text-align: center;"><i class="mdi mdi-18px mdi-spin mdi-loading"></i> Mengambil data . . .</td></tr>')
            	$('#pagination').hide()
              load()
            },
            success:function(data){
		...
		function dataAo (data) {
			$('#selectAreaKerja').prop('disabled',false)
      $('#search').prop('disabled',false)
			if (data.amount == 0) {
          $('#pagination').hide()
    			$('#showMemoAo').html('<tr><td colspan="8" style="text-align: center;"><i class="mdi mdi-close"></i> Data Kosong !!</td></tr>')
    	}else {
          $('#pagination').show()
    			var html = '';
					var i;
					for(i=0; i<data.length; i++){
							var ButPrint = '<a href="'+url+'Report/ReportMemoAO/'+data[i].id_order+'" target="_blank"><label class="but badge badge-danger"><i class="mdi mdi-printer"></i>&nbsp;Cetak</label></a>&nbsp;'

							var butDetail = '<a href="javascript:void(0)" id="detailmemoao" dataIdOr="'+data[i].id_order+'" dataIdCdeb="'+data[i].id_calon_debitur+'" dataIdKc="'+data[i].id_kredit_checking+'"><label class="but badge badge-warning"><i class="mdi mdi-account-box"></i>&nbsp;detail</label></a>&nbsp;'
							...
							html += '<tr>'+
											...
											'<td>'+data[i].nama_calon_debitur+'</td>'+
											verifikasi+
											'<td>'+data[i].type_kredit+'</td>'+
											'<td>'+data[i].jenis_fasilitas+'</td>'+
											'<td>'+
												ButPrint+
												butDetail+
												butEdit+
		...
		function totalData() {
			$.ajax({
				type:'GET',
				url:url+'home/getTotalAllAo',
				dataType:'JSON',
				success:function(res) {
					if (res.total_all_ao > 10) {
						let datapage = $('.next-page').attr('data')
						let lastpage = res.total_all_ao - datapage
